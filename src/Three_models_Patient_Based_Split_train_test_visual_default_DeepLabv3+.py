...
def build_model(MODEL, ENCODER_NAME):
    ...
    if MODEL == "DeepLabV3Plus":
        return smp.DeepLabV3Plus(
            ...
            in_channels=3,
            classes=1,
            decoder_dropout={"p": 0.3},
            decoder_aspp_dropout={"p": 0.5}
        )
...
def calculate_metrics(pred, mask, threshold=THRESHOLD, use_logits=False):
    smooth = 1e-6
...
if __name__ == "__main__":
    ...
    dataset_path = r"Path to the local dataset"
    ...
    train_loader = DataLoader(train_set, batch_size=12, shuffle=True, num_workers=2, pin_memory=True)
    val_loader = DataLoader(val_set, batch_size=12, shuffle=False, num_workers=2, pin_memory=True)
    ...
    print("ENCODER:", model.encoder.__class__ if hasattr(model, "encoder") else "NO ENCODER")
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch {epoch + 1}/{num_epochs}, "f"Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, "f"Train Acc: {train_accuracy:.4f}, Val Acc: {val_accuracy:.4f}, "f"Train IoU: {train_iou:.4f}, Val IoU: {val_iou:.4f}")
    ...
    model.load_state_dict(
        torch.load(str(NUMEPOCS) + "_best_model_" + MODEL + ".pth", weights_only=True, map_location=device))
    model.eval()

...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Epoch {epoch + 1}/{num_epochs}, "f"Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, "f"Train Acc: {train_accuracy:.4f}, Val Acc: {val_accuracy:.4f}, "f"Train IoU: {train_iou:.4f}, Val IoU: {val_iou:.4f}")
    ...
    with open(test_log_file, "a", newline="") as f:
        csv.writer(f).writerow(
            ["MEAN_RESULTS", np.mean(precision_list), np.mean(recall_list), np.mean(f1_list), np.mean(iou_list),

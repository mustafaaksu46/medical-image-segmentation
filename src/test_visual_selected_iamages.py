...
# The dataset index numbers of the images you want to test will be written here.
...
def test_and_visualize(dataset_path, model_path, model_name=MODEL, num_epochs=NUMEPOCS,
    ...
    # Create dataset (Use all data from SELECTED_INDICES for testing)
    dataset = MedicalDataset(image_paths, mask_paths, transform=val_transform)

    print(f"Total dataset size: {len(dataset)}")

    # If specific indices are provided, use only those
    if selected_indices is not None:
        print(f"\nTesting only selected {len(selected_indices)} indices: {selected_indices}")

        # Validate indices
        valid_indices = [idx for idx in selected_indices if 0 <= idx < len(dataset)]
        invalid_indices = [idx for idx in selected_indices if idx < 0 or idx >= len(dataset)]
        ...
        print(f"Testing {len(valid_indices)} images from the dataset")

        # Create a subset with only selected indices
        from torch.utils.data import Subset
        test_set = Subset(dataset, valid_indices)
    else:
        # Use entire dataset
        test_set = dataset
        print(f"Testing all {len(dataset)} images")

    test_loader = DataLoader(test_set, batch_size=1, pin_memory=True)  # batch_size=1 for easier index tracking
